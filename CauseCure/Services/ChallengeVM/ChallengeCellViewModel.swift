...
class ChallengeCellViewModel: ObservableObject, Identifiable {
    ...
    static func newChallenge() -> ChallengeCellViewModel {
        print("here")
        return ChallengeCellViewModel(challenge: Challenge(title: "", category: "", durationDays: 7, interval: "", searchName: [""], description: "", completed: false, challengeCreater: Auth.auth().currentUser?.uid ?? "" ))
    }
    
    static func newChallenge(title: String, durationDays: Int, interval: String, searchName: [String], description: String, completed: Bool, challengeCreater: String) -> ChallengeCellViewModel {
        print("here")
